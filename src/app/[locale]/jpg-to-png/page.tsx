...
export default async function LocalizedJpgToPng({ params }: Props) { const { locale: value } = await params; if (!isLocale(value) || value === "ko") notFound(); const copy = homeCopy[value]; return <section className="tool-hero"><div className="shell tool-page-shell"><div className="tool-title"><div className="eyebrow">{copy.eyebrow}</div><h1>{copy.cta}</h1><p>{copy.description}</p></div><JpgToPngConverter locale={value as Locale} /></div></section>; }
